...
# it has to be a 2 class problem
data(Titanic)

Titanic <- as.data.frame(Titanic)

# put the data into train and test set
train_index <- createDataPartition(y=Titanic$Survived, p=0.7, list=FALSE)
training_set <- Titanic[train_index, ]
testing_set <- Titanic[-train_index, ]

myLadder <- CreatLadder(804, pRatio=0.75, Nmin=5 )

# have to fix, gets a NaN/infinite value error
#RSVM(Titanic, y=c("Yes","No"), myLadder, CVtype="LOO", CVnum=804 )
