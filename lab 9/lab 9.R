...
# first we check if the classes are linearly separable.
plot(x, col=(3-y))
# They're not, so next we fit the support vector classifier.
...
#use summary to get the cross-validation errors for each of these models
summary(tune.out)
...
# Now one additional observation is misclassified.
